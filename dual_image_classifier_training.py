...
class DualImageCNNClassifier(nn.Module):
    def __init__(self, pretrained=True):
        super(DualImageCNNClassifier, self).__init__()
        
        # Load pre-trained ResNet18 models for each input image
        self.encoder1 = models.resnet18(pretrained=pretrained)
        self.encoder2 = models.resnet18(pretrained=pretrained)
        
        # Remove the final fully connected layer
        self.encoder1 = nn.Sequential(*list(self.encoder1.children())[:-1])
        self.encoder2 = nn.Sequential(*list(self.encoder2.children())[:-1])
        
        # Create a new classifier for combined features
        self.classifier = nn.Sequential(
            nn.Linear(512 * 2, 512),  # 512 features from each encoder
            nn.BatchNorm1d(512),
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(512, 128),
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            nn.Linear(128, 2)  # Binary classification: benign or malignant
        )
...
def main():
    ...
    root_dir = '/home/s2751455/uusirr/miccai_2022_buv_dataset/rawframes'  # Update this with your dataset path
    ...
    # Initialize model
    model = DualImageCNNClassifier(pretrained=True)
    model.to(device)
    
    # Optimizer and scheduler
    optimizer = optim.Adam([
        {'params': model.encoder1.parameters(), 'lr': 1e-5},  # Lower learning rate for pretrained encoders
        {'params': model.encoder2.parameters(), 'lr': 1e-5},
    ...
    hyperparameters = {
        'model': 'Dual ResNet18 CNN Classifier',
        'Learning Rate for encoders': 1e-5,
        'Learning rate for classifier': 1e-3,
        'Weight Decay': 1e-5,
        'Batch Size': 16,
        'Epochs': epochs,
    ...
    # Get all test predictions for confusion matrix
    model.eval()
    all_test_preds = []
    all_test_labels = []
